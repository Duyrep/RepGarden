          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mt-4">
            {metrics.map((metric, index) => (
              ...
              >
                ...
                */}
                <MetricCard {...metric}>
                  <div className="mt-auto pt-3 border-t border-surface-a10 w-full flex justify-end gap-1">
                    {/* Nút Xóa */}
                    <Button
                      variant="ghost"
                      className="text-surface-a40 hover:text-danger-a0 hover:bg-danger-a0/10 flex items-center gap-2 h-8 px-3 text-sm rounded-lg transition-colors"
                      aria-label="Xóa chỉ số"
                      onClick={(e) => {
                        e.preventDefault(); // Ngăn sự kiện nổi bọt
                        if (
                          window.confirm(
                            `Bạn có chắc chắn muốn xóa chỉ số "${metric.name}" không?`,
                          ) &&
                          metric.id
                        ) {
                          appDB.metrics.delete(metric.id);
                        }
                      }}
                    >
